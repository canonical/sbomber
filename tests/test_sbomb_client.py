...
from sbomber import DEFAULT_MANIFEST, DEFAULT_PACKAGE_DIR, prepare, DEFAULT_STATEFILE, submit
...
def sbomber_post_mock(project: Path):
    def get_mm(url, *args, **kwargs):
        ...
        if url.endswith("/upload"):
            mm.json.return_value = {"data": {"artifactId": "foo"}}
        return mm
...
def test_submit(project, tmp_path, sbomber_get_mock, sbomber_post_mock, secscanner_run_mock):
    ...
    submit()
    assert not sbomber_get_mock.called
    assert sbomber_post_mock.call_count == 3
