...
from clients.client import ProcessingStatus, ArtifactType
from clients.sbom import SBOMber, DEFAULT_SERVICE_URL
from clients.secscanner import Scanner

logger = logging.getLogger("sbomber")
DEFAULT_SBOMBERPATH = Path(__file__).parent.resolve()
DEFAULT_STATEFILE = DEFAULT_SBOMBERPATH / ".statefile.yaml"
DEFAULT_MANIFEST = DEFAULT_SBOMBERPATH / "manifest.yaml"
...
def submit(statefile: Path = DEFAULT_STATEFILE,
            pkg_dir: Path = DEFAULT_PACKAGE_DIR):
    ...
    clients = {
        SBOMB_KEY: SBOMber(
            ...
            team=team,
            service_url=meta.get("sbom-service-url")
        ), SECSCAN_KEY: Scanner()
    }
...
def poll(statefile: Path = DEFAULT_STATEFILE,
          wait: bool = False,
          timeout: int = 15):
    meta = yaml.safe_load(statefile.read_text())
    try:
